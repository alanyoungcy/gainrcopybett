...
/* ═══════════════════════════════════════════════════════════
   MOCK DATA — Analyst Profiles (MVP Fields)
   ═══════════════════════════════════════════════════════════ */
...
const liveMarkets = [
  {
    ...
    history: [
      { t: '6h', y: 48 }, { t: '5h', y: 50 }, { t: '4h', y: 46 },
      { t: '3h', y: 52 }, { t: '2h', y: 49 }, { t: 'Now', y: 54 },
    ],
  },
  {
    ...
    history: [
      { t: '6h', y: 58 }, { t: '5h', y: 55 }, { t: '4h', y: 57 },
      { t: '3h', y: 50 }, { t: '2h', y: 53 }, { t: 'Now', y: 52 },
    ],
  },
  {
    ...
    history: [
      { t: '6h', y: 50 }, { t: '5h', y: 53 }, { t: '4h', y: 48 },
      { t: '3h', y: 55 }, { t: '2h', y: 52 }, { t: 'Now', y: 57 },
    ],
  },
  {
    ...
    history: [
      { t: '6h', y: 47 }, { t: '5h', y: 50 }, { t: '4h', y: 48 },
      { t: '3h', y: 52 }, { t: '2h', y: 49 }, { t: 'Now', y: 51 },
    ],
  },
];
...
const CARD = 'bg-[#141922] rounded-[20px] border border-white/[0.06]';
...
export default function Home() {
  ...
  const renderDiscover = () => (
    <div className="px-6 md:px-8 lg:px-10 pt-3 pb-36 md:pb-12">
      {/* Search */}
            ...
            <div className="p-6 pb-0">
              ...
              <div className="grid grid-cols-4 gap-2 mb-5">
                {[
                  ...
                  { label: 'Win Rate', value: `${analyst.winRate}%`, color: 'text-white', sub: analyst.winRate >= 65 ? 'Favs' : analyst.winRate >= 50 ? 'Mixed' : 'Longshots' },
                ].map((m, i) => (
            ...
            <div className="px-6 pb-6">
              {selectedAnalyst === analyst.id ? (
                <div className={`${CARD_INNER} p-5`}>
                  <p className="text-[13px] font-semibold text-white/70 mb-3">Set Copy Amount</p>
                  {/* Preset Buttons */}
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleCopy(analyst.id)}
                      className="flex-1 py-3.5 bg-orange-500 text-white rounded-[12px] font-bold text-[15px] hover:bg-orange-400 active:scale-[0.98] transition-all">
                      Confirm Copy
                    </button>
                    <button onClick={() => setSelectedAnalyst(null)}
                      className="px-4 py-3.5 rounded-[12px] bg-white/5 border border-white/[0.06] text-white/40 hover:text-white/60 hover:bg-white/10 transition-all">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <p className="text-[11px] text-white/30 mt-3 flex items-center gap-1.5">
                    <Target className="w-3 h-3" /> Bets mirror this analyst proportionally
                  </p>
                </div>
              ) : (
                <button
                  ...
                  className={`w-full py-3.5 rounded-[12px] font-bold text-[15px] transition-all ${
                    copiedAnalysts.includes(analyst.id)
                ...
                >
                  {copiedAnalysts.includes(analyst.id) ? 'Copying' : 'Copy Analyst'}
                </button>
              )}
            </div>
          </div>
            ...
            <div className="p-6">
              ...
              <div className={`${CARD_INNER} p-3 mb-4`}>
                <ResponsiveContainer width="100%" height={70}>
                  <LineChart data={mkt.history}>
                    ...
                    />
                    <Line type="monotone" dataKey="y" stroke={mkt.trend === 'up' ? '#10b981' : '#ef4444'} strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                >
                  ...
                  <p className="text-[32px] font-extrabold tracking-tighter text-emerald-400 leading-none">{mkt.yesPrice}<span className="text-[16px]">¢</span></p>
                ...
                >
                  ...
                  <p className="text-[32px] font-extrabold tracking-tighter text-red-400 leading-none">{mkt.noPrice}<span className="text-[16px]">¢</span></p>
                  <p className="text-[11px] text-white/30 mt-1.5">
                ...
                <div className={`${CARD_INNER} p-5`}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <p className={`text-[22px] font-extrabold tracking-tighter ${betSide === 'yes' ? 'text-emerald-400' : 'text-red-400'}`}>
                      {betSide === 'yes' ? mkt.yesPrice : mkt.noPrice}¢
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 mb-4 px-1">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[15px] font-bold text-white/50">
                        ${((parseFloat(betAmount) || 0) * (100 / (betSide === 'yes' ? mkt.yesPrice : mkt.noPrice)) * 0.98).toFixed(2)}
                      </span>
                    </div>
                  </div>
                  {/* Action Buttons — Stacked */}
                  <button onClick={() => handleBet(mkt.id)}
                    className="w-full py-3.5 bg-orange-500 text-white rounded-[12px] font-bold text-[15px] hover:bg-orange-400 active:scale-[0.98] transition-all shadow-[0_0_20px_rgba(249,115,22,0.15)] mb-2">
                    Place Bet
                  </button>
                  <button onClick={() => setSelectedMarket(null)}
                    className="w-full py-3 rounded-[12px] bg-white/5 border border-white/[0.06] text-white/40 text-[14px] font-semibold hover:text-white/60 hover:bg-white/10 transition-all">
                    Cancel
                  </button>
                </div>
              )}
              ...
              {placedBets.includes(mkt.id) && selectedMarket !== mkt.id && (
                <div className="flex items-center gap-3 bg-emerald-500/10 rounded-[12px] p-4 border border-emerald-500/20">
              ...
              return (
                <div key={id} className={`${CARD} p-5 flex items-center gap-4`}>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[18px] font-extrabold text-white tracking-tight">${copyAmount}</p>
                ...
                <div key={id} className={`${CARD} p-5`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-[18px] font-extrabold text-white tracking-tight">${betAmount}</p>
                      <div className="flex items-center gap-1 justify-end mt-0.5">
        ...
        <div className={`${CARD} p-12 text-center`}>
          ...
          <div className="flex gap-2.5 justify-center">
            <button onClick={() => setTab('discover')}
              className="px-5 py-3 bg-orange-500 text-white rounded-[12px] text-[14px] font-bold hover:bg-orange-400 transition-colors">
              Browse Analysts
            </button>
            <button onClick={() => setTab('markets')}
              className="px-5 py-3 bg-white/5 border border-white/[0.06] text-white/50 rounded-[12px] text-[14px] font-bold hover:bg-white/10 transition-colors">
              View Markets
            </button>
          </div>
        </div>
  ...
  /* ─── Tab: Profile ─── */
  const renderProfile = () => (
    <div className="px-6 md:px-8 lg:px-10 pt-3 pb-36 md:pb-12 md:max-w-xl md:mx-auto">
      {/* User Card */}
      ...
      <div className={`${CARD} overflow-hidden`}>
        {[
          { icon: <Settings className="w-5 h-5 text-white/40" />, label: 'Settings', sub: 'Preferences' },
          { icon: <HelpCircle className="w-5 h-5 text-white/40" />, label: 'Help & Support', sub: 'FAQs & contact' },
  ...
  return (
    <div className="min-h-screen bg-[#0d1117]">
      ...
      <header className="sticky top-0 z-50 bg-[#0d1117]/80 backdrop-blur-xl border-b border-white/[0.04]">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-8 lg:px-10 h-14 md:h-16">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
        <div className="max-w-lg mx-auto px-5 pb-3">
          <div className="bg-[#141922]/90 backdrop-blur-xl rounded-[18px] border border-white/[0.06] shadow-[0_-4px_30px_rgba(0,0,0,0.3)] px-2 py-1.5 flex items-center justify-around">
            {tabs.map(({ key, label, icon: Icon }) => {
              const active = tab === key;
              return (
                <button
